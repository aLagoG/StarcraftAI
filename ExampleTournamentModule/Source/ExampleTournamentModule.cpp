...
void ExampleTournamentAI::onStart() {
    // Set the command optimization level (reduces high APM, size of bloated
    // replays, etc)
...
void ExampleTournamentAI::onReceiveText(BWAPI::Player player,
                                        std::string text) {}
...
bool ExampleTournamentModule::onAction(BWAPI::Tournament::ActionID actionType,
                                       void* parameter) {
    switch(actionType) {
        ...
        case Tournament::EnableFlag:
            ...
            // Allow other flags if we add more that don't affect gameplay
            // specifically
            return true;
        ...
        case Tournament::SetCommandOptimizationLevel:
            return *(int*)parameter >
                   MINIMUM_COMMAND_OPTIMIZATION;  // Set a minimum command
                                                  // optimization level to
                                                  // reduce APM with no action
                                                  // loss
        default:
            break;
    }
