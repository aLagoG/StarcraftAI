...
int main(int argc, const char* argv[]) {
    ...
    while(true) {
        ...
        if(hMod == NULL) {
            ...
            PFNCreateA1 newAIModule =
                (PFNCreateA1)GetProcAddress(hMod, "newAIModule");

            if(!newGame || !newAIModule) {
                std::cerr << "ERROR: Failed to find AI Module exports"
                          << std::endl;
                client = new AIModule();
        ...
        while(Broodwar->isInGame()) {
            for(std::list<Event>::const_iterator e =
                    Broodwar->getEvents().begin();
                e != Broodwar->getEvents().end(); ++e) {
